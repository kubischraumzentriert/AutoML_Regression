...
| 27 Missing-Data-Mechanismus-Audit (MCAR/MAR/MNAR) - ist Fehlen informativ statt naiv median-/mode-zu-imputieren? | Klassifikation+Regression (Modul, 13 synthetische Checks je Template) + `beijing-air-quality-panel` (1 reales Projekt) | in Arbeit (2026-09-14) - `missingness_mechanism_audit.R` in beiden Templates (identisch, baut auf `univariate_drift.R` auf: "missing vs. nicht-missing" = strukturell derselbe Vergleich wie "Train vs. Test"). Reale Anwendung an Beijing zeigt einen wichtigen Reibungspunkt: bei grossem n (360k) wird fast jede Spalte "signifikant" (CO: KS-D=0,02, aber p_adj=2,9e-05) - Modul braucht noch einen optionalen Mindest-Effektgroessen-Schwellenwert, um bei grossen Datensaetzen "signifikant" von "praktisch relevant" zu trennen. Noch nicht ADR-003-reif (nur 1 reales Projekt, Verfeinerung offen) |
| 28 Quantil-/Verteilungsregression als Alternative/Ergaenzung zu `conformal_prediction.R` | keins (0) | offen (2026-09-14) - Pinball-Loss-Modelle (`regr.lightgbm` mit `objective="quantile"` o.ae.) statt/zusaetzlich zu Split-Conformal, Vergleich von Coverage und Intervallbreite (Schaerfe) auf denselben Daten |
