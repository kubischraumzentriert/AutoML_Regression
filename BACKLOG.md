...
| 27 Missing-Data-Mechanismus-Audit (MCAR/MAR/MNAR) - ist Fehlen informativ statt naiv median-/mode-zu-imputieren? | Klassifikation+Regression (Modul, 24/20 synthetische Checks je Template) + `beijing-air-quality-panel` + `openml-house-prices-regression` (2 reale Projekte, strukturell verschiedene Mechanismen, inkl. Verfeinerung) | erledigt (2026-09-14, 2. Zeuge 2026-09-19) - `missingness_mechanism_audit.R` in beiden Templates (identisch, baut auf `univariate_drift.R` auf). Optionaler `min_effect_size`-Schwellenwert ergaenzt (KS-D/Cramers V zusaetzlich zur Signifikanz) nach realem Reibungsfund (bei n=360k war fast jede Spalte "signifikant"); erneuter Beijing-Lauf mit `min_effect_size=0.1` differenziert klar zwischen 7 Spalten mit echtem Ziel-Effekt (D>=0,14) und 3 Spalten mit nur statistischem, keinem praktisch relevanten Signal (CO/SO2/NO2, D<=0,065). **2. Projekt-Zeuge (`openml-house-prices-regression`, 2026-09-19)**: strukturell ANDERER Mechanismus (informative Abwesenheit - "kein Pool"/"keine Garage"/"kein Keller" als leeres Feld kodiert - statt Beijings Sensorluecken) UND ~300x kleinere Stichprobe (n=1.168 statt 360.966). 17/18 Spalten zeigen substanzielle Effektgroessen (KS-D 0,17-0,64, deutlich ueber der Schwelle, plausibel echt statt stichprobengroessen-getrieben); die eine Ausnahme (`MiscFeature`, kein signifikanter Ziel-Hinweis trotz MAR-Signal) zeigt, dass das Werkzeug nicht pauschal alles meldet. ADR-003-Kriterium fuer die `min_effect_size`-Verfeinerung damit erfuellt (Modul selbst war bereits backportiert). Details: `ML_Learning/openml-house-prices-regression/README.md` Abschnitt "Kandidat 27". |
...
| 29 Negative Stacking-Gewichte (JOSS-Kandidat #8, `stacks`-Mechanik) - 2. unabhaengiger Test | Klassifikation (1, s6e9, 2x Nullbefund) + `electricity-load-panel` (1, `035_negative_stacking_weights.R`, 15-Modelle-Pool, erste Regressions-Anwendung) | erledigt (2026-09-14), ADR-003 erfuellt - erneuter Nullbefund, identisches Muster: 0 von 15 negative Koeffizienten trotz Erlaubnis, RMSE 122,53 fuer beide Stacking-Varianten identisch, bestes Einzelmodell (RMSE 120,20) schlaegt beide. 2 unabhaengige Projekte (Klassifikation+Regression), beide negativ - kein Backport, `JOSS_TECHNIQUE_WATCH.md` Kandidat #8 final auf Prioritaet niedrig gesetzt |
| 30 Robuste Loss-Funktionen bei extremen Ziel-Ausreissern (Huber/Quantil statt reinem RMSE) | `electricity-load-panel` (1, `034_robust_loss_functions.R`, Faktor >400 Lastunterschied) | erledigt (2026-09-14) - sauberer Negativbefund: bei korrekt skaliertem `alpha` UND ausreichendem Iterationsbudget unterscheiden sich Huber/Quantile-Median praktisch NICHT von L2/RMSE (Ratio 0,72/0,65, auch nicht bei Worst-Case-Fehlern). Wichtigerer Fund als das Nullergebnis: eine generische LightGBM-Falle (Default-`alpha`=0.9 + L2-kalibriertes Iterationsbudget laesst Huber faelschlich katastrophal aussehen, reine Konvergenzgeschwindigkeit) - als Warnhinweis in `WORKFLOW_GUARDS.md` zurueckgefuehrt |
| 31 Multi-Horizont-Forecasting - ein Modell fuer mehrere Vorlaufzeiten gleichzeitig statt separater Modelle je Horizont | `beijing-air-quality-panel` + `electricity-load-panel` (2, `036_multi_horizon_forecasting.R` je Projekt) | erledigt (2026-09-15, 2. Zeuge 2026-09-19) - "horizon-as-feature"-Ansatz (Zeilen gestapelt, bei 24h-Vorlauf nicht bekannte Spalten NA-maskiert, EIN Modell). Beijing: 1h-Horizont RMSE +1,41 schlechter (23,63 vs. 22,22), 24h-Horizont RMSE -0,69 besser (63,42 vs. 64,11). **2. Projekt-Zeuge (`electricity-load-panel`, 2026-09-19)**: zuerst die fehlende 24h-Horizont-Referenz nachgezogen (`027_forecast_24h_reference.R`, LightGBM RMSE 168,86 vs. Persistence 221,53 - echtes Signal), dann Multi-Horizont-Modell: 1h RMSE +10,04 schlechter (122,51 vs. 112,47), 24h RMSE -5,83 besser (163,03 vs. 168,86) - **exakt derselbe qualitative Trade-off wie bei Beijing** (kurzer Horizont wird schlechter, langer wird besser), deutlich groessere Betraege, aber gleiche Richtung in beiden unabhaengigen Projekten. ADR-003-Kriterium erfuellt. **Kein Backport als Modul**: die Stapel-/NA-Masken-Technik selbst ist ~10 Zeilen direkter Code, was projektuebergreifend gilt ist der EMPIRISCHE Befund (kein klarer Gewinner, Betriebsvorteil vs. letzte RMSE-Punkte), nicht ein wiederverwendbarer Baustein. Details: `ML_Learning/electricity-load-panel/README.md` Abschnitt 10. |
| 32 Concept-Drift ueber mehrere Zeitperioden (rollierend statt ein einzelner Train/Test-Split) | Klassifikation+Regression (Modul, 13/11 synthetische Checks je Template) + `beijing-air-quality-panel` (1 reales Projekt) | erledigt (2026-09-16) - neues `rolling_drift_diagnosis.R` (beide Templates, identisch, baut auf `univariate_drift.R` auf): `assign_time_windows()` + `rolling_univariate_drift()` + `drift_trend_test()` (Spearman-Trend-Test, unterscheidet systematischen Trend von einmaligem Fenster-Rauschen). Reale Anwendung an Beijing (8 Zeitfenster ueber 4 Jahre): Drift-Staerke oszilliert klar zwischen Sommer (~0,08-0,14) und Winter (~0,23-0,24), Trend-Test findet exakt rho=0,000 - reine Saisonalitaet, kein Klimatrend. Genau die Unterscheidung demonstriert, fuer die das Modul gebaut wurde |
